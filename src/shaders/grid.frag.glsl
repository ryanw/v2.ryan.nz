...
float edgeDistance(vec3 barycentric) {
	...
	vec3 d = sqrt(dx * dx + dy * dy) * 3.0;
	//vec3 d = fwidth(barycentric) * 3.0;
	vec3 a = smoothstep(d - 0.005, d, barycentric);
	return min(min(a.x, a.y), a.z);
}

void main(void) {
	vec3 c0 = vec3(1.0, 0.8, 0.0);
	vec3 c1 = vec3(0.3, 0.0, 0.8);
	vec3 color = mix(c0, c1, edgeDistance(vBarycentric));
